...
def parse_rate_limit(headers: Dict[str, str]) -> RateLimitInfo:
...
def maybe_sleep_for_rate_limit(headers: Dict[str, str], minimum_remaining: int = 2) -> None:
...
def github_request(url: str, token: Optional[str]) -> Tuple[dict, Dict[str, str]]:
    headers = {
    ...
    try:
        with urlrequest.urlopen(req) as resp:
            ...
            # Convert headers to a plain dict with str keys
            resp_headers = {k: v for k, v in resp.headers.items()}
            return data, resp_headers
    except HTTPError as e:
        if e.code in (403, 429):
            # Try to sleep until reset then retry once
            resp_headers = {k: v for k, v in (e.headers.items() if e.headers else [])}
            maybe_sleep_for_rate_limit(resp_headers, minimum_remaining=1)
            with urlrequest.urlopen(req) as resp:
                body_bytes = resp.read()
                body_text = body_bytes.decode("utf-8")
                data = json.loads(body_text)
        ...
        # Brief backoff for transient network issue
        time.sleep(1.5)
        with urlrequest.urlopen(req) as resp:
            body_bytes = resp.read()
            body_text = body_bytes.decode("utf-8")
...
def search_total_count(star_filter: str, token: Optional[str]) -> Tuple[int, Dict[str, str]]:
    # Use per_page=1 to cheaply get total_count
    params = {
        "q": f"stars:{star_filter}",
        "per_page": "1",
    }
    url = f"{SEARCH_REPOS_ENDPOINT}?{urlparse.urlencode(params)}"
    data, headers = github_request(url, token)
    total_count = int(data.get("total_count", 0))
    return total_count, headers


def search_items(star_filter: str, token: Optional[str]) -> Iterator[dict]:
    # Paginate through up to 1,000 results for a given filter (<= 1,000 ensured by caller)
    per_page = 100
    total, headers = search_total_count(star_filter, token)
    maybe_sleep_for_rate_limit(headers)
    if total == 0:
        return
...
def find_global_max_stars(token: Optional[str]) -> int:
    ...
    items = data.get("items", [])
    if not items:
        # Fallback to a safe high number if unexpected
        return 1_000_000
    top = items[0]
    return int(top.get("stargazers_count", 1_000_000))


def binary_find_upper_bound_for_range(min_exclusive: int, global_max: int, token: Optional[str]) -> int:
    ...
    while low <= high:
        ...
        count, headers = search_total_count(star_filter, token)
        maybe_sleep_for_rate_limit(headers)
        if count == 0:
            low = mid + 1
            continue
...
def iter_all_repos_over_min_stars(min_stars: int, token: Optional[str], max_repos: Optional[int] = None) -> Iterator[dict]:
...
def normalize_repo_record(item: dict) -> dict:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--format", type=str, choices=["jsonl", "csv"], default="jsonl", help="Output format")
    ...
    if args.format == "jsonl":
        ...
        write_csv(args.out, gen_records())
